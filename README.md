...
## Project Overview
Cardiovascular disease (CVD) is the leading cause of death globally, with nearly 650 million people living with the condition. The objective of this project is to determine if standard, easily obtainable clinical data can be used to precisely predict heart failure using machine learning. 
...
* **Handling Biological Anomalies:** 172 patient records contained a biologically impossible cholesterol level of 0 mm/dl. To preserve 20% of the dataset, these were rescued using median imputation rather than deletion.
...
| :--- | :--- | :--- | :--- | :--- |
| **1** | **Scikit-Learn Baseline (Random Forest)** | 0.8824 | 0.8867 | Strong baseline, but missed 12 actively sick patients. |
| **2** | Naive Deep TF Baseline (Unregularized) | 0.8725 | 0.8683 | Severely overfit the small dataset; missed 13 patients. |
| **3** | Lower Learning Rate (Adam 0.0001) | 0.8922 | 0.8966 | Prevented rapid conclusions; improved clinical sensitivity. |
| **4** | Functional API (Wide & Shallow 128) | 0.8824 | 0.8867 | Stable architecture, but failed to beat the low learning rate. |
| **5** | Uniform Dropout Regularization (0.3) | 0.8824 | 0.8696 | Built redundant diagnostic pathways. |
| **6** | L2 Weight Decay Penalty (0.01) | 0.8922 | 0.8922 | Continuous mathematical tax successfully raised Recall. |
| **7** | **Early Stopping + Dropout + Low LR** | **0.9216** | **0.8952** | **Ultimate Model: Missed only 8 sick patients**. |
